...
// NODE_ID -> normalized compute node id (first compute node in patch has normalized id of 0),
// NBR_ID -> stencil neighbor id, COMP_ID -> component id
...
#ifdef STRIPE_OPT
#ifdef RUN_32BIT
    #define SVM_LIN_ID(PID, SV_ID) ((SV_ID) * LPSMD->patch_size + ((PID) < LPSMD->patch_size/2 ? (PID)*2 : \
                                                                                ((PID) - LPSMD->patch_size/2) * 2 + 1))
#else
    #define SVM_LIN_ID(PID, SV_ID) ((SV_ID) * LPSMD->patch_size + (PID))
#endif
    #define CFDL_SVM_LIN_ID(PID, SV_ID) ((4 * (PID)) + (SV_ID))
#else
#ifdef USE_CFDL
...
typedef struct PSMD_struct {
    ...
    fType CQ_POOL(CQ_POOL_Dx)* Dx;
    fType CQ_POOL(CQ_POOL_Dy)* Dy;
    fType CQ_POOL(CQ_POOL_Dz)* Dz;
    fType CQ_POOL(CQ_POOL_L)* L;
} PSMD_struct;

#ifdef CACHEQ
// Need to make an identical copy with CACHEQ alignment and canonicalized pointers that will be passed to the
// dataflow
typedef struct CQ_PSMD_struct {
    int lnbr_rank;
    int rnbr_rank;
    int patch_size, compute_size, lhalo_size, rhalo_size;            // local patch total, compute and halo layer sizes
    int halo_pid_s, compute_pid_s, compute_pid_e, halo_pid_e;        // halo and compute layer start/stop node ids
...
typedef struct timing_struct {
    ...
    double CQ_POOL(TIMER_POOL)* t_main;
    double CQ_POOL(TIMER_POOL)* t_eval_rhs;
    double CQ_POOL(TIMER_POOL)* t_mpi;
    double CQ_POOL(TIMER_POOL)* t_eval_K;
    double CQ_POOL(TIMER_POOL)* t_update_D;
    double CQ_POOL(TIMER_POOL)* t_update_H;
