#!/bin/bash
stripeopt="-DSTRIPE_OPT"
ds=$(echo "x$*" | fgrep 'DSTRIPE_OPT')
if [[ "$ds" != "" ]]; then 
   stripeopt="-DSTRIPE_OPT"
fi
flags="${stripeopt} -DCACHEQ -DUSE_BINIO -DUSE_CFDL -DPADDED_NUM=4 -DSIMD_LENGTH=4 -DRUN_32BIT -Iinclude -I$CACHEQ_SOURCE_DIR/libcq"
gccfiles="main.c profiling.c runtime_params.c input.c rcm.c reorder_nodes.c layout.c matrix_transformations.c init_patches.c halos.c"
   set ofiles = ""
   foreach f ($gccfiles)
